...
# load already-aggregated first-24h dataframe (one row per ICU stay)
df = pd.read_parquet("data/eicu_first24.parquet")  # replace with your path

# splits (example: 5 hospitals train, 1 held-out H6)
...
# fit selector on TRAIN ONLY (prevents leakage)
...
# train a simple LR baseline (you can swap for LightGBM later)
